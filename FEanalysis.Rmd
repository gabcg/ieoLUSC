...
We will perform this three times: with and without SVA, and also without the first SV
...
Here is a list of the top 10 genes that were found to be differentally expressed across the gene sets gathered from the GSVA.

A total of 4128 genes among the gene sets were found to be differentially expressed after the GVSA.
...
After finding differentially expressed genes, the next step was to complete a gene ontology analysis. This procedure was completed to see if the DE genes were associated with certain biological processes defined by the Gene Ontology system. 

The first step in this process is build a parameter object specifying the gene universe of interest, the set of DE genes, the ontology, etc. The ontology selected was "BP", which matches genes to the Biological Processes associated with the GO Terms.
...
In GO analysis, GO terms can sometime overlap. Therefore, a conditional test was chosen. The number of significant GO terms identified was .......
```{r}
...
Since signficiantly enriched gene sets formed by a few genes are not very reliable, a filter was added so those sets with less than or equal to 3 genes were removed. The table was than ordered by Odds Ratio. If the Odds Ratio has "Inf", this indicates that all genes within a gene set are differentially expressed.
...
First, the DEgenes gathered from the differential expression analysis without sva are loaded.
```{r}
DEgenes <- readRDS(file.path("results", "DEgenes_noSVA.rds"))
```
...
First, the DEgenes gathered from the differential expression analysis without sva are loaded.
```{r}
DEgenes <- readRDS(file.path("results", "DEgenes_noSV1.rds"))
```
