...
length(c4)
```
After the collection was selected for the GSEA, the next step was to map the identifiers from the xxx gene sets to the data set being analyzed.
```{r}
...
gsc
```
A matrix was created to check if the idenitifers were properly mapped. The incidence matrix below inidcates what genes belong to the data collection and the data set being analyzed. A 1 denotes that the gene is a member of that data set, while a 0 denotes that the gene is not a part of that data set. 
```{r}
Im <- incidence(gsc)
dim(Im)
Im[1:2, 1:10]
...
dim(Im)
```
Since we are only interested in the genes from the data set being analyzed, all genes that are not a part of this gene set, denoted by a 0, are discarded. This leaves 5663 genes to be analyzed.
```{r}
...
With the t-statistics avaialble, the z-scores were then calculated to see if a shift in gene expression within a gene set was present. As shown below, the z-scores are all quite close to 0. The highest z-score shown is around 3, indicating this gene set is about 3 standard deviations above the mean. A low z-score suggests the genes in the gene set are not differentially expressed.(not sure if this last sentence is correct)
```{r}
Im <- Im[rowSums(Im) >= 5, ]
dim(Im)
...
A one sample z-test was calculated and a conservative multiple testing adjustment was administered to see if any gene sets were candidates for differentally expressed genes. As shown below, no gene sets were ....
```{r}
...
Since there doesn't appear to be an difference in how the genes are behaving between the two groups, a gene set variation analysis (GSVA) was completed. This differs from the standard GSEA, because a GSVA characterizes the underlying biological pathway activity rather than the behavior of functionally related genes. The goal is to see how the biological pathway activity compares among the two groups. 
...
All the gene sets showing differentally expressed genes were used in another differential expression analysis. Below, the genes showing the highest t-value are shown. 
```{r}
...
head(tt, n = 10)
```
A total of 561 genes were found to be differentally expressed across the gene sets gathered from the GSVA. 
```{r}
