...
The summary table displays the up-regulated, not significant, and down-regulated genes for each variable in the model.
...
downDEgenes <- rownames(tt.sign)[tt.sign$logFC < -2]
upDEgenes <- rownames(tt.sign)[tt.sign$logFC > 2]
...
So finally `r length(DEgenes)` genes calling differentially expressed, of which `r length(downDEgenes)` are down-regulated and `r length(upDEgenes)` up-regulated. As the chromosome information was added to the table of results, the chromosome distribution of these genes can be obtained. 
...
Regarding the QQ plot, it represents the quantiles of the data sample against the theoretical distribution they should follow. The diagonal represents the null hypothesis. Most genes are off-diagonal, indicating differential expression. The fact that most of the genes are far from the null hypothesis is a measure of the significance of the results.

The differential expression results are diagnosed with volcano and MA plots, in which the top 10 differentially expressed genes are highlighted as shown in Figure \@ref(fig:volcano-ma-plots). Volcano plots are scatter plots that summarize a measure of significance, such as the p-values (which can be in logarithmic scale as the plot below), and the log-fold change, which represents the measure of differential expression. Significant results will appear towards the plot, while fold changes increase to right for up-regulated genes and to left for down-regulated genes.

In the case of the MA plot, the log-fold change is plotted in the y axis, while the average expression of the samples is on the x axis. In this plot, it is expected to see the results centered around zero. Red dots represent significantly differentially expressed genes that are above 2 or below -2 log-fold change.

The top differentially expressed genes are tagged for both plots.

```{r volcano-ma-plots, out.width="800px", fig.cap="Volcano and MA plots for DE analysis."}
...
Both the volcano and MA plot show that the results do not have problems such as unexpected trends.
