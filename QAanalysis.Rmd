...
* `r length(medium_c)` samples with moderate differences from the mean, which we define as ( M <= 2 and M >= 1 or M >=-2 and M <= -1). Their regression line is purple.
* `r length(small_c)` samples with moderate differences from the mean as they are (M < 1 or M > -1). Their regression line is green.
...
* `r length(medium_n)` samples with moderate differences from the mean, which we define as ( M <= 2 and M >= 1 or M >=-2 and M <= -1). Their regression line is purple.
* `r length(small_n)` samples with moderate differences from the mean as they are (M < 1 or M > -1). Their regression line is green.
...
Finally, under the consideration that there is no batch effect, the 24 samples that were removed in order to make the dendogram and the MDS plot will remain for further analysis. We decide to generate the dendogram in \@ref(fig:allSampleClustering) again in order to see if any other sample appears in its opposite cluster, apart from `8623-tumor`.

```{r allSampleClustering, fig.height=7, fig.width=14, dpi=100, echo=TRUE, fig.cap="Figure S6: Hierarchical clustering of the samples."}
