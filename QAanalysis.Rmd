...
Lung cancer, specifically lung squamous cell carcinoma, was assessed and analyzed in this experiment. In this dataset, a total of xxx tumors were submitted to the Cancer Genome Atlas Program (TCGA).

We start importing the raw table of counts. From this, we see that 20115 genes from a total of 553 samples are in this data.
...
Next, we explored the column (phenotypic) data, which in this case corresponds to clinical
variables, and their corresponding metadata. For example, the column "type" consists of tumor and normal labels. Other information such as gender, tumor stage, and smoking status are also shown.

In any case, we can access all the possible labels with the last line of code, which returns two columns of information about the clinical variables. One called `labelDescription` contains a succint description of the variable, often
...
Next, we will look at the distribution of expression values per sample in terms of
logarithmic CPM units. We display tumor and normal samples separately, and are shown
in Figure \@ref(fig:distRawExp). Each colored line in the graphs below represent a sample.
In both graphs, we see two modes. The low mode represents genes not expressed in that sample.
The second mode represents the genes that are expressed.
...
for (i in 1:ncol(setmp)) {
  ...
  lo <- lowess(M ~ A)
  lines(lo$x, lo$y, col= if(any(lo$y > 2) || any(lo$y < -2)){"red"} else if (any(lo$y <= 2 & lo$y >= 1) || any(lo$y >= -2 & lo$y <= -1)){"purple"} else if(any(lo$y < 1) || any(lo$y > -1)){"green"}, lwd=2)

if(any(lo$y > 2) || any(lo$y < -2)){ big_c_c <- append(big_c,samplename)} else if (any(lo$y <= 2 & lo$y >= 1) || any(lo$y >= -2 & lo$y <= -1)){ medium_c <- append(medium_c,samplename)} else if(any(lo$y < 1) || any(lo$y > -1)){small_c <- append(small_c,samplename)}
}
```

In the MA-plots of the tumorous samples, we found `r length(big_c) ` samples with some expresion very different of the mean ( M > 2 or M < -2), with the regresion line in red. `r length(medium_c)` samples with some expresion moderatly different from the mean ( M <= 2 and M >= 1 or M >=-2 and M <= -1), with the regresion line in purple. And finally `r length(small_c)` samples that have an expresion-level without any indices of dependent biases (M < 1 or M > -1), with the regresion line in green.
...
In the MA-plots of the normal samples, we found `r length(big_n) ` samples with some expresion very different of the mean ( M > 2 or M < -2), with the regresion line in red. `r length(medium_n)` samples with some expresion moderatly different from the mean ( M <= 2 and M >= 1 or M >=-2 and M <= -1), with the regresion line in purple. And finally `r length(small_n)` samples that have an expresion-level without any indices of dependent biases (M < 1 or M > -1), with the regresion line in green.
...
```
  ...
  * Samples were sequenced within the different plates, which could lead to a batch effect.
  * Samples of normal cells have two different portion of analyte, while the tumor samples have five different portions.

We are going to use the portion of analyte as surrogate of batch effect indicator. Considering our outcome
of interest as molecular changes between sample types, tumor vs. normal, we will examine now
the cross-classification of this outcome with portion of analyste.
...
scaling, annotating the outcome of interest and the surrogate of batch indicator, or portion of analyte in our case. We calculate again log CPM values with a higher prior count to moderate extreme fold-changes produced by low counts. The resulting dendrogram is shown in Figure \@ref(fig:sampleClustering).
...
We can observe that samples cluster primarily by sample type, tumor or normal. The portion of analyte seems to have a stronger effect among the normal samples, while it distributes better among the tumor samples. We may consider discarding samples leading to an unbalanced distribution of the outcome across batches.
...
<!--We can also observe that two tumor
